...
def window_partition(x, window_size):
    ...
    Returns:
        windows: (num_windows*B, window_size,
        window_size, C)
    """
    B, H, W, C = x.shape
    x = x.view(
        B, H // window_size, window_size, W // window_size,
        window_size, C
    )
...
def window_reverse(windows, window_size, H, W):
    ...
    x = windows.view(
        B, H // window_size, W // window_size, window_size,
        window_size, -1
    )
...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        window_size (tuple[int]): The height and width of
        the window.
        num_heads (int): Number of attention heads.
        qkv_bias (bool, optional):  If True, add a
        learnable bias to query, key, value. Default: True
        attn_drop (float, optional): Dropout ratio of
        attention weight. Default: 0.0
        proj_drop (float, optional): Dropout ratio of
        output. Default: 0.0
        pretrained_window_size (tuple[int]): The height
        and width of the window in pre-training.
    """
    ...
    def __init__(
        ...
        relative_coords[:, :, 0] *= 2 * self.window_size[
            1] - 1
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: input features with shape of (
            num_windows*B, N, C)
        ...
        q, k, v = qkv[0], qkv[1], qkv[
            2]  # make torchscript happy (cannot use
        # tensor as tuple)
        ...
        logit_scale = torch.clamp(
            self.logit_scale,
            max=torch.log(torch.tensor(1. / 0.01)).to(
                self.logit_scale.device
            )
        ).exp()
    ...
    def extra_repr(self) -> str:
        return f'dim={self.dim}, window_s' \
               f'ize={self.window_size}, ' \
               f'pretrained_windo' \
               f'w_size={self.pretrained_window_size}, ' \
               f'num_heads={self.num_heads}'

    def flops(self, N):
        # calculate flops for 1 window with token length
        # of N
        flops = 0
        # qkv = self.qkv(x)
...
class SwinTransformerBlock(nn.Module):
    ...
    Args:
        ...
        mlp_ratio (float): Ratio of mlp hidden dim to
        embedding dim.
        qkv_bias (bool, optional): If True, add a
        learnable bias to query, key, value. Default: True
        drop (float, optional): Dropout rate. Default: 0.0
        attn_drop (float, optional): Attention dropout
        rate. Default: 0.0
        drop_path (float, optional): Stochastic depth
        rate. Default: 0.0
        act_layer (nn.Module, optional): Activation
        layer. Default: nn.GELU
        norm_layer (nn.Module, optional): Normalization
        layer.  Default: nn.LayerNorm
        pretrained_window_size (int): Window size in
        pre-training.
    """
    ...
    def __init__(
        ...
        if min(self.input_resolution) <= self.window_size:
            # if window size is larger than input
            # resolution, we don't partition windows
            self.shift_size = 0
    ...
    def calculate_mask(self, x_size):
        ...
        h_slices = (slice(0, -self.window_size), slice(
            -self.window_size, -self.shift_size
        ), slice(-self.shift_size, None))
        w_slices = (slice(0, -self.window_size), slice(
            -self.window_size, -self.shift_size
        ), slice(-self.shift_size, None))
        cnt = 0
        for h in h_slices:
    ...
    def forward(self, x, x_size):
        ...
        # W-MSA/SW-MSA (to be compatible for testing on
        # images whose shapes are the multiple of window
        # size
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, input_resolution=" \
               f"{self.input_resolution}, num" \
...
class BasicLayer(nn.Module):
    ...
    Args:
        ...
        norm_layer (nn.Module, optional): Normalization
        layer. Default: nn.LayerNorm
        use_checkpoint (bool): Whether to use
        checkpointing to save memory. Default: False.
        pretrained_window_size (int): Local window size
        in pre-training.
    """

    def __init__(
            self, dim, input_resolution, depth, num_heads,
            window_size, mlp_ratio=2., qkv_bias=True,
        ...
        self.blocks = nn.ModuleList(
            [SwinTransformerBlock(
                ...
                num_heads=num_heads,
                window_size=window_size, shift_size=0 if (
    ...
    def extra_repr(self) -> str:
        return f"dim={self.dim}, " \
               f"input_reso" \
               f"lution={self.input_resolution}, depth=" \
               f"{self.depth}"
...
class PatchEmbed(nn.Module):
    ...
    Args:
        img_size (int): Image size.  Default: 126.
        patch_size (int): Patch token size. Default: 1.
        in_chans (int): Number of input image channels.
        Default: 180.
        embed_dim (int): Number of linear projection
        channels. Default: 180.
        norm_layer (nn.Module, optional): Normalization
        layer. Default: None
    """

    def __init__(
            self, img_size=126, patch_size=1, in_chans=180,
            embed_dim=180, norm_layer=None
    ):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # assert H == self.img_size[0] and W ==
        # self.img_size[1],
        #     f"Input image size ({H}*{W}) doesn't match
        #     model ({self.img_size[0]}*{self.img_size[
        #     1]})."
...
class RSTB(nn.Module):
    ...
    Args:
        ...
        resi_connection: The convolutional block before
        residual connection.
    """
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            ...
            in_chans=dim, embed_dim=dim, norm_layer=None
        )
...
class Upsample(nn.Sequential):
    ...
    Args:
        ...
        and 3.
        num_feat (int): Channel number of intermediate
        features.
    """
...
class UpsampleOneStep(nn.Sequential):
    """UpsampleOneStep module (the difference with
    Upsample is that it always only has 1conv +
    1pixelshuffle)
    ...
    def __init__(
            self, scale, num_feat, num_out_ch,
...
class Net(nn.Module):
    ...
    Args:
        ...
        Default: 180
        depths (tuple(int)): Depth of each Swin
        Transformer layer.
        num_heads (tuple(int)): Number of attention heads
        in different layers.
        window_size (int): Window size. Default: 7
        mlp_ratio (float): Ratio of mlp hidden dim to
        embedding dim. Default: 2
        ...
        embedding to the patch embedding. Default: False
        patch_norm (bool): If True, add normalization
        after patch embedding. Default: True
        use_checkpoint (bool): Whether to use
        checkpointing to save memory. Default: False
        img_range: Image range. 1. or 255.
        resi_connection: The convolutional block before
        residual connection. '1conv'/'3conv'
    """

    def __init__(
            self, img_size=126, patch_size=1, in_chans=3,
            embed_dim=180, depths=[6, 6, 6, 6, 6, 6],
            num_heads=[6, 6, 6, 6, 6, 6], window_size=7,
            mlp_ratio=2., qkv_bias=True, drop_rate=0.,
            attn_drop_rate=0., drop_path_rate=0.1,
            norm_layer=nn.LayerNorm, ape=False,
            patch_norm=True, use_checkpoint=False,
            img_range=255., resi_connection='1conv',
            **kwargs
    ):
        ...
        if in_chans == 3:
            ...
            self.mean = torch.Tensor(rgb_mean).view(
                1, 3, 1, 1
            )
        else:
        ...
        self.patch_embed = PatchEmbed(
            ...
            norm_layer=norm_layer if self.patch_norm else
            None
        )
        ...
        # build the last conv layer in deep feature
        # extraction
        if resi_connection == '1conv':
    ...
    def check_image_size(self, x):
        ...
        mod_pad_w = (self.window_size - w %
                     ...
                     self.window_size
        x = F.pad(
            x, (0, mod_pad_w, 0, mod_pad_h), 'reflect'
        )
        return x
...
#     upscale = 4
#     window_size = 8
#     height = (1024 // upscale // window_size + 1) *
#     window_size
#     width = (720 // upscale // window_size + 1) *
#     window_size
